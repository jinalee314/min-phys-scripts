...
class TDIntensityxz : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  Communicator cm;
...
public:
  explicit TDIntensityxz(const ActionOptions&);
  ~TDIntensityxz();
// active methods:
...
PLUMED_REGISTER_ACTION(TDIntensityxz,"TDINTENSITYXZ")

void TDIntensityxz::registerKeywords( Keywords& keys ){
...
TDIntensityxz::TDIntensityxz(const ActionOptions&ao):
...
// Communicator
  ...
  if(gb_lista.size()>0){
    ...
    if(doneigh)  nl= new NeighborList(ga_lista,serial,pbc,getPbc(),cm,nl_cut,nl_st);
    else         nl= new NeighborList(ga_lista,serial,pbc,getPbc(),cm);
  }
...
TDIntensityxz::~TDIntensityxz(){
  delete nl;
}

void TDIntensityxz::prepare(){
  if(nl->getStride()>0){
   ...
   for(unsigned int i=rank;i<nn;i+=stride) {
    ...
    if(pbc){
     ...
     distance=delta(getPosition(i0),getPosition(i1));
      ...
      distancezModulo=std::sqrt(distance[1]*distance[1]);
