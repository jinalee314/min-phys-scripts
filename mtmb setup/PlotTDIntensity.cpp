...
class PlotTDIntensityno : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  Communicator cm;
...
public:
  explicit PlotTDIntensityno(const ActionOptions&);
  ~PlotTDIntensityno();
// active methods:
...
PLUMED_REGISTER_ACTION(PlotTDIntensityno,"PLOTTDINTENSITYNO")

void PlotTDIntensityno::registerKeywords( Keywords& keys ){
  ...
  keys.add("compulsory","ZCUT","1","Wavelength of the incident wave");
  keys.add("compulsory","ZMAXR","1","Wavelength of the incident wave");
  keys.add("compulsory","DCUT","1","Wavelength of the incident wave");
  ...
  keys.add("compulsory","ZLOW","1","Number of bins of Q ");
  keys.add("compulsory","ZUP","1","Number of bins of Q ");
...
PlotTDIntensityno::PlotTDIntensityno(const ActionOptions&ao):
...
// Communicator
  ...
  if(gb_lista.size()>0){
    if(doneigh)  nl= new NeighborList(ga_lista,gb_lista,serial,dopair,pbc,getPbc(),cm,nl_cut,nl_st);
    else         nl= new NeighborList(ga_lista,gb_lista,serial,dopair,pbc,getPbc(),cm);
  } else {
    if(doneigh)  nl= new NeighborList(ga_lista,pbc,serial,getPbc(),cm,nl_cut,nl_st);
    else         nl= new NeighborList(ga_lista,pbc,serial,getPbc(),cm);
  }
...
PlotTDIntensityno::~PlotTDIntensityno(){
  delete nl;
}

void PlotTDIntensityno::prepare(){
  if(nl->getStride()>0){
...
// calculator
void PlotTDIntensityno::calculate()
{
...
//     theta.resize(qhist);
  ...
  unsigned rank=comm.Get_rank();
    ...
    double distancezModulo;
    Vector distance_versor;
    Vector derivsf;

    double deltad=dcut/qbin;
    double fij_O;
    N_atom=getNumberOfAtoms();
   double temp_int;
   double temp_der;
   temp_int=0;
   temp_der=0;
...
//    double q_A=4*pi*std::sin(theta[m]*pi/180)/(lamda*10);
  ...
  double predistanceM= deltad*(j+0.5);
       temp_int=2*fij_O*fij_O*std::tr1::cyl_bessel_j(0,q*predistanceM)/getNumberOfAtoms();
       temp_der=2*fij_O*fij_O*q*std::tr1::cyl_bessel_j(1,q*predistanceM)/getNumberOfAtoms();
    if (predistanceM>maxr){
    preintensity[j]=temp_int*(1-(predistanceM-maxr)/(dcut-maxr));
    prederivsf[j]=(1+maxr/(dcut-maxr))*temp_der - (temp_int+temp_der*predistanceM)/(dcut-maxr);
   } else
    {
   preintensity[j]=temp_int;
   prederivsf[j]=temp_der;
    }
   }
   ...
   for(unsigned int i=rank;i<nn;i+=stride) {
    ...
    if(pbc){
     ...
     distance=delta(getPosition(i0),getPosition(i1));
      ...
      distancezModulo=std::sqrt(distance[2]*distance[2]);
//    if ((pos0[2]>zlow && pos0[2]<zup) || (pos1[2]>zlow && pos1[2]<zup)) {
      if (distancezModulo<zcut) {  
    //log.printf("distncemodulo= %f\n",distancezModulo);
   // log.printf("distncez= %f\n",distance[2]);
     if(distancezModulo<zmaxr)
     {
     af=1;
     } else{
     af=1-(distancezModulo-zmaxr)/(zcut-zmaxr);
     }
      distance[2]=0.0;
      distance_versor = distance / distanceModulo;
      distance_versor[2]=0.0;

     if(distanceModulo<dcut){
     int q_num=distanceModulo/deltad;
      value[m]+=2*af*(preintensity[q_num]);
      derivsf=2*af*prederivsf[q_num]*distance_versor;
   }
...
//   }
       ...
       comm.Sum(sfVirial);
         ...
         structureF[m]=2*fij_O*fij_O+value[m];
//  log.printf("value2d, fij, %f,%f \n",value[m],fij_O*fij_O);
         for(unsigned i=0;i<getNumberOfAtoms();++i) setAtomsDerivatives(valueSF[m],i,sfPrime[m][i]);
//    log.printf(" 2theta and Intensity %f, %f\n",2*theta[m],structureF[m]);

         valueSF[m]->set(structureF[m]);
         setBoxDerivatives  (valueSF[m],sfVirial[m]);
