...
MODIFIED OCT 2025 by jinalee314
...
class GDSAnalyzer(object):
    def __init__(self,
             ...
             xyz='merge.xyz',
             mode='mass',
             nw=2, # for MgO-Fe system, we use 1, for Pv-H system, we use 2
             project_axis = 2,
             begin = 0,
             end = -1,
             step = 1,
        ...
        self.analyze_mol_frac = analyze_mol_frac
        self.analyze_planar = analyze_planar

        if self.analyze_mol_frac:
    ...
    def save_sum(self):
        ...
        # add wc surface area output
        # Format frame index and surface area as two columns
        dat = np.column_stack((np.arange(self.begin, self.end), np.array(self.sa)))
        fmt = '%d %.4f'
        np.savetxt(os.path.join(os.getcwd(),'wc_surface_area.txt'), dat, fmt=fmt, header='willard chandler surface_area(A^2)')
        self.plot_mol_frac()
        self.plot_prox()
        self.plot_gds()
    ...
    def analyze(self):
        ...
        for fn in range(self.begin,self.end,self.step):
            ...
            self.sum_elements_counts.append(sol_liq_inter) # corresponds to sum_counts
            self.sum_phase_dimension.append([ls, ll, lw, lx,ly,lz, z0, z1_unpbc]) # corresponds to sum_dimensions
    ...
    def analyze_mol_frac_single(self):
        ...
        np.savetxt(os.path.join(os.getcwd(),'molfrac/'+str(self.current)+'_atomic_fraction.txt'),np.array(self.mol_frac), header='{0} \n {1}'.format(self.xyz,'           '.join(ele_list) ))
    ...
    def plot_mol_frac(self):
        ...
        plt.savefig(str(self.current)+'_atomic_fraction_vs_proximity.png',dpi=300,bbox_inches='tight')
    ...
    def analyze_single(self,idx,solid_center0=-1):
        ...
        self.sa.append(self.inter.surface_area)         
    ...
    def save_prox(self):
        ...
        """
        np.savetxt(os.path.join(os.getcwd(),'density/'+str(self.current) + '_prox.txt'), np.array([self.mean_prox,self.mean_rho]).T, header='{0} \n {1}'.format('proximity','density'))
        ...
        # Just iterate through the total number of atoms (rows), without extracting coordinates
        combined_lines = [self.inter.all_atoms.elements[i] + ' ' + str(self.proximity[i]) +' ' + str(phase_array[i]) + '\n' for i in range(self.inter.all_atoms.n_atoms)]
        
        # save raw proximity as k value
        # decide if needed (only want the raw proximity, not the coordinates. also, for each frame)
        out = open(os.path.join(os.getcwd(),'raw/'+str(self.current)+'_raw.txt'),'w')
        out.writelines(combined_lines)
        out.close()

        # Obtain raw count of atoms of each element type along the proximity profile
            # Bin edges based on self.mean_prox so profiles line up (this outputs at end)
        self.prox_bin_edges = np.concatenate([
            [self.mean_prox[0] - (self.mean_prox[1] - self.mean_prox[0])/2],
            0.5 * (self.mean_prox[1:] + self.mean_prox[:-1]),
            [self.mean_prox[-1] + (self.mean_prox[-1] - self.mean_prox[-2])/2]
        ])
        self.prox_counts = {}

        for ele in self.inter.unique_ele:
            ele_mask = np.array(self.inter.all_atoms.elements) == ele
            ele_prox = np.array(self.proximity)[ele_mask]
            counts, _ = np.histogram(ele_prox, bins=self.prox_bin_edges)
            self.prox_counts[ele] = counts.astype(int)

        # Optionally, save to file (for each frame)
        np.savetxt(os.path.join(os.getcwd(),'counts/'+str(self.current) + '_prox_counts.txt'),
                       np.vstack([self.mean_prox] + [self.prox_counts[ele] for ele in self.inter.unique_ele]).T,
                       header='proximity ' + ' '.join(self.inter.unique_ele))
    ...
    def save_prox_xyz(self,outname='prox.xyz'):
        ...
        # Optionally, save to file (for each frame)
        np.savetxt(str(self.current) + '_prox_counts.txt',
                       np.vstack([self.mean_prox] + [self.prox_counts[ele] for ele in self.inter.unique_ele]).T,
                       header='proximity ' + ' '.join(self.inter.unique_ele))
    ...
    def plot_prox(self,out='prox.png'):
        ...
        yrange = [min(self.mean_rho)-0.5, max(self.mean_rho)+0.5]
        plt.figure()
        ...
        plt.ylim(yrange)
        plt.xlim(xrange)
        plt.xlabel('Proximity'+' ' + r'$(\mathrm{\AA})$', fontsize=14)
        plt.ylabel(r'$\rho$'+' ' + r'$(\mathrm{g}/\mathrm{cm}^{3})$', fontsize=14)
        plt.minorticks_on()
        plt.savefig(str(self.current) + '_' + out,dpi=300,bbox_inches='tight')
